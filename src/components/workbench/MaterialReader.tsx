    ...
    <header className="border-b border-border-warm pb-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="font-heading text-xl text-text-main leading-snug truncate">
            {material.title || material.filename}
          </h1>
          <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-text-muted">
            <span className="inline-block px-1.5 py-px border border-red-primary/60 text-red-primary text-[10px]">
...
function TabBar({
  ...
  return (
    <nav className="flex gap-0 border-b border-border-warm overflow-x-auto scrollbar-hide">
      {tabs.map((tab) => (
        <button
          key={tab.key}
          ...
          className={[
            'px-3 py-2 text-xs whitespace-nowrap transition-colors relative -mb-px font-body shrink-0',
            activeTab === tab.key
